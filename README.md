...
O backend pode ser executado via Docker (ambiente isolado) ou localmente para desenvolvimento. Em ambos os casos, as migrações do banco de dados são aplicadas automaticamente ao iniciar a aplicação.

#### **Opção 1: Via Docker Compose (Recomendado)**

Esta opção sobe a API e o banco de dados PostgreSQL sincronizados.

1.  **Pré-requisitos:** Docker e Docker Compose instalados.
2.  **Execução:** Na raiz do projeto (`/controle-de-gastos/`), execute:
    ```bash
...
1.  **Pré-requisitos:** [.NET SDK 9.0](https://dotnet.microsoft.com/download/dotnet/9.0) e PostgreSQL instalados.
2.  **Configuração do Banco:**
    - Ajuste a connection string em `backend-dotnet/backend-dotnet/appsettings.json` para apontar para seu banco local.
3.  **Execução:**
    - Navegue até a pasta do projeto:
      ```bash
      cd backend-dotnet/backend-dotnet
      ```
    - Execute a aplicação:
      ```bash
...
4.  **Acesso:**
    ...
    - **Swagger:** `http://localhost:5124/swagger/index.html`
...
3.  **Execução da Aplicação:**
    ...
    **Observação:** O frontend está configurado para se comunicar com o backend na porta `8080` (via Docker). Se necessário, ajuste a URL base das requisições no arquivo `src/core/url.ts`. Use assim caso esteja rodando localmente e não via Docker.
    ```ts
    export const API_BASE_URL = "http://localhost:5124";
    ```
